#!/usr/bin/env python3
"""Initialize, update, check, or install the NM V4 workflow."""
...
DEFAULT_REF = "main"
MANIFEST_PATH = "template/v4/manifest.json"
...
def run(command: list[str], cwd: Path, *, check: bool = True) -> subprocess.CompletedProcess[str]:
    ...
    if check and result.returncode != 0:
        ...
        raise NmV4Error(f"command failed: {' '.join(command)}\n{message}")
    return result
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Manage the NM V4 workflow template.")
    ...
    for name in ("init", "update", "check"):
        ...
        if name == "update":
            sub.add_argument(
                "--branch",
                help="Branch to create before updating. Defaults to chore/sync-nm-workflow-v4-YYYYMMDD.",
            )
            sub.add_argument(
                "--allow-dirty",
                action="store_true",
...
def resolve_target(value: str, *, create: bool) -> Path:
    ...
    if not target.exists() or not target.is_dir():
        raise NmV4Error(f"target directory does not exist: {target}")
    return target


def validate_relative_path(value: Any) -> str:
    if not isinstance(value, str) or not value:
        raise NmV4Error(f"invalid manifest path: {value!r}")
    path = Path(value)
    if path.is_absolute() or value == "." or ".." in path.parts:
        raise NmV4Error(f"manifest path must stay inside project: {value}")
    return value
...
def read_source_file(relative: str, *, source_dir: Path | None, ref: str) -> bytes:
    if source_dir:
        ...
        if not path.is_file():
            raise NmV4Error(f"missing template source: {path}")
    ...
    try:
        ...
        raise NmV4Error(f"failed to download {url}: {exc}") from exc
...
def load_manifest(*, source_dir: Path | None, ref: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise NmV4Error(f"invalid manifest JSON: {exc}") from exc
    for key in ("directories", "templates"):
        if not isinstance(manifest.get(key), list):
            raise NmV4Error(f"manifest missing list: {key}")
    return manifest
...
def load_state(target: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise NmV4Error(f"invalid {STATE_FILE}: {exc}") from exc
...
def ensure_git_for_update(target: Path, *, allow_dirty: bool, branch: str | None, dry_run: bool) -> None:
    ...
    if root is None or root != target:
        raise NmV4Error("update requires target to be the root of a Git repository")
    status = run(["git", "status", "--short"], target).stdout.strip()
    if status and not allow_dirty:
        raise NmV4Error("working tree has changes; commit or stash before update")
    ...
    if exists:
        raise NmV4Error(f"branch already exists: {branch_name}")
    if dry_run:
...
def default_update_branch() -> str:
    ...
    return f"chore/sync-nm-workflow-v4-{datetime.now().strftime('%Y%m%d')}"
...
def install_skill(args: argparse.Namespace) -> None:
    ...
    skill_source = source_dir / "skills" / "nm-init-project-v4"
    if not skill_source.is_dir():
        raise NmV4Error(f"missing skill source: {skill_source}")
    if args.target_dir:
    ...
    destination = target_root / "nm-init-project-v4"
...
def print_migration(moved: list[str], skipped: list[str], hints: list[str]) -> None:
    ...
    for value in hints:
        print(f"  - {value}")
    if hints:
        print("Next: synthesize a V4 Spec from these documents with the administrator, then confirm it.")
