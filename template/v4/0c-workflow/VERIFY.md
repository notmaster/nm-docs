...
阶段完成的定义：`./0d-scripts/verify.sh` 通过，且 ROADMAP 中该阶段的 `Verify:` 命令全部通过。

`verify.sh` 默认先调用 `check-workflow.sh`，检查 V4 目录结构、Spec 命名、ROADMAP 状态文件和脚本执行权限。
