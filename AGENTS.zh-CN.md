...
- V3.1 是新项目的推荐工作流。推荐状态只决定默认工作流选择；它不代表生产就绪，也不授权受保护分支、推送、发布、部署、生产访问或其他外部影响。
...
- 仅在实质状态变化时发送 `progress`：完成预检后开始工作，以及完成一个有意义的中间阶段。不要发送例行心跳、每条命令一条通知，或对未变化状态重复通知。
- 最终完成并通过验证时发送 `attention`，作为对管理员的显式交接提醒；因需要管理员决策或授权、安全或 secret 风险、不安全的 Git 状态或冲突、修复或验证路径耗尽，或其他阻止安全推进的 blocker 而停止前，也发送 `attention`。
