#!/usr/bin/env python3
"""Locate and run the NM V4 tool from nm-docs."""
...
RAW_TOOL_URL = "https://raw.githubusercontent.com/notmaster/nm-docs/main/tools/nm-v4/nm_v4.py"
...
def find_tool() -> Path | None:
    for start in candidate_repos():
        ...
        for path in [current, *current.parents]:
            tool = path / "tools" / "nm-v4" / "nm_v4.py"
            if tool.is_file():
                return tool
    return None
...
def download_tool() -> Path:
    target = Path(tempfile.gettempdir()) / "nm_v4.py"
